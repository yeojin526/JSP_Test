...
<script>
	alert("이미 로그인 되어있습니다.");
	window.location.href = "list_view.jsp"; // 게시판 페이지로 이동
</script>
<%
} else {
...
if (cookies != null && cookies.length > 0) { //쿠키 배열값에 데이터가 존재할 때
	...
	checked = "checked"; //브라우저에 응답한다. (새로고침 느낌?)
...
<body class="d-flex align-items-center py-4 bg-body-tertiary">
  ...
  <path
	...
	<div
		...
		<div class="position-fixed top-0 start-0 mb-3 me-3">
			<button class="btn btn-border-subtle btn-sm"
				...
				<span class="me-1"> <!-- 아이콘과 텍스트 사이 간격 조절 --> 
	...
	<main class="form-signin w-100 m-auto">
		<form action="<%=request.getContextPath()%>/sessionLoginProc.jsp" method="post" >
			<img class="mb-4" src="assets/brand/balloon-heart-fill.svg" alt=""
				width="72" height="57">
			...
			<div class="form-floating">
				<input type="email" name="userpw" class="form-control"
					id="floatingPassword" value="<%=userpw%>" placeholder="name@example.com"> <label
					for="floatingPassword">이메일</label>
			</div>

			<div class="form-check text-start my-3">
				<input class="form-check-input" type="checkbox" name="remember"
					id="flexCheckDefault" value="chk" <%=checked%>> <label
					class="form-check-label" for="flexCheckDefault"> 아이디 기억하기 </label>
			</div>

			<button class="btn btn-danger w-100 py-2" type="submit"
				onclick="location.href='sessionLoginProc.jsp'">로그인</button>
